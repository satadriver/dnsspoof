...
int parse_dns_query(uint8_t *buf, query *queries,res_record *answers, res_record *auth, res_record *addit)
{
	//BEGIN_SOLUTION
	dns_header *dns = NULL;
	dns = (dns_header*)buf;
...
#else
			char * strip4=inet_ntoa(*(struct in_addr*)answers[i].rdata);
			strcpy(ip4, strip4);
#endif
...
int get_dns_name(uint8_t *dns, uint8_t *host)
{
...
//	printf("host name: %s\n", host_cp);
	...
	char *tk = strtok(host_cp, ".");
	int i = 0;
	while(tk!=NULL)
	{
		int sublen = strlen(tk);

		*(dns+i) = (uint8_t)(sublen); //set the number of chars in the label

		i++;
		
		strncpy((char*)(dns+i), tk, sublen); //the label
...
void build_dns_header(dns_header *dns, int id, int query, int qd_count,int an_count, int ns_count, int ar_count)
{
	srand(time(NULL));
...
{
	int nl = get_dns_name(qname, (uint8_t*)host_name);
	*position = nl; 
}
